...
| `replicate()` | Rejected with `UnsupportedProtectedOperation`: a copy drops the primary key but keeps the foreign keys and loses the context, so its relations would load around every policy | `tests/Query/ProtectionClosureTest.php::test_a_copy_of_a_protected_model_is_refused` |
| a model's own `$with` | Not applied, on the root query or on eager-loaded related models: those loads would go around the related model's policy. Name the relation in `with([...])` instead | `tests/Query/ProtectionClosureTest.php::test_a_models_own_eager_loads_do_not_go_around_the_related_policy` |
| a model's own `$withCount` | Rejected with `UnsupportedProtectedOperation` when the protected query is built: the aggregate would count related rows the viewer may not see | `tests/Query/ProtectionClosureTest.php::test_a_model_level_aggregate_is_refused` |
| an eager-loaded `HasMany` | A `ProtectedCollection` of protected models, exactly like the lazy path; every model reachable through a loaded relation is bound to the same context | `tests/Query/ProtectionClosureTest.php::test_an_eager_loaded_has_many_is_a_protected_collection` |
| `select([...])` together with `with([...])` | The key columns the eager load matches on are always added to the select, so a visible parent never reads as `null` because its key was not selected | `tests/Query/ProtectionClosureTest.php::test_an_eager_belongs_to_survives_a_select_that_omits_the_foreign_key` |
...
| a denial inside a caller's transaction | The auditor runs while the caller's transaction is still open (not after it); an audit on the same connection is lost if the caller rolls back | `tests/Action/NestedTransactionTest.php::test_a_denial_is_audited_inside_the_callers_transaction_when_nested` |
